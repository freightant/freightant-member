...
import { getCountry, getOrg, getSessionCache, getUser, signUPEndPoint2, signUPEndPoint3, uploadFile } from '@/network/endpoints'
...
const Licenses = ({ setCurrentStep }: { setCurrentStep: Dispatch<SetStateAction<number>> }) => {
    ...
    const { data, error, isLoading } = useSWR("/", getOrg)
    const [form] = Form.useForm();
    const onFinish = async (value: any) => {
        let errorLists: string[] = []
        let minDoc = false
        ...
        let res = CheckpointCheck(value.userdocuments)

        if (!res.result) {
            res.cp?.forEach((i, iIndex) => {
                if (!i) {
                    errorLists.push("Please upload at least one document from Group " + (iIndex + 1))
                }
            })
        }
